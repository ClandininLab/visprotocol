...
class MhtProtocol(ClandininLabProtocol.ClandininLabProtocol):
    ...
    def getEpochParameters(self, protocol_ID, protocol_parameters, epoch):
        ...
        if protocol_ID == 'CheckerboardWhiteNoise':
        ...
        elif protocol_ID == 'StationaryMapping':
            stimulus_ID = 'MovingPatch'

            stim_time = self.run_parameters['stim_time'] #sec
            flash_duration = protocol_parameters['flash_duration'] #sec
            
            time_steps = np.arange(0,stim_time,flash_duration)
            no_steps = len(time_steps)
            x_steps = np.random.choice(protocol_parameters['azimuth_locations'], size = no_steps, replace=True)
            y_steps = np.random.choice(protocol_parameters['elevation_locations'], size = no_steps, replace=True)
            ...
            convenience_parameters = {'square_width':protocol_parameters['square_width'],
                                      'angle':0,
                                      'color':protocol_parameters['color'],
                                      'elevation_locations':protocol_parameters['elevation_locations'],
                                      'azimuth_locations':protocol_parameters['azimuth_locations'],
                                      'flash_duration':protocol_parameters['flash_duration'],
                                      'x_steps':x_steps,
                                      'y_steps':y_steps,
                                      'time_steps':time_steps}
            
        elif protocol_ID == 'SparseBinaryNoise':
            stimulus_ID = 'MovingPatch'
